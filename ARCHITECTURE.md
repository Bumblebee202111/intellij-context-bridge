...
* **Tool Call & Diff Manager:** Relies on automated native UI actions (e.g., "Copy as Markdown") to reliably extract sanitized AI responses. Parses incoming tool-specific XML tags (e.g., `<ide:read_file>`, `<ide:propose_edit>`) via the `AgentActionParser` to route back to the UI for manual approval. Code payloads are strictly wrapped in `<![CDATA[...]]>` to prevent XML entity escaping and indentation corruption. The system employs a "Code Block Jailbreak" and XML namespacing (`ide:`) to prevent the LLM's Tool Classifier from hijacking the response into a native UI function-call chunk, ensuring tools are extracted as raw text.
