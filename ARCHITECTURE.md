...
## 5. Command Engine
* **Slash Commands:** A user-invoked macro system for workflow state transitions (e.g., `/plan`, `/commit`). Commands can be stacked. The UI inserts tokens (e.g., `/plan`), and the backend dynamically extracts and interpolates them, wrapping multiple directives in distinct XML semantic boundaries (`<applied_commands>`) before appending the user's specific manual request.
