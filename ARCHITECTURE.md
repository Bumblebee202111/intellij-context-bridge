...
* **Tool Call & Diff Manager:** Relies on automated native UI actions (e.g., "Copy as Markdown") to reliably extract sanitized AI responses. Parses incoming XML-based tool calls (e.g., `read_file`, `propose_edit`, `rename_file`) to route back to the UI for manual approval, while matching code blocks to local file paths to open IntelliJ's native side-by-side `DiffRequest` window or Refactoring APIs.

## 5. Command Engine
* **Slash Commands:** A user-invoked macro system for workflow state transitions (e.g., `/plan`, `/review`). Commands are stacked and interpolated at the backend layer, keeping the UI clean while wrapping multiple directives in distinct XML semantic boundaries (e.g., `<command name="plan">`) before appending the user's specific request.
