...
* **File Tree View:** Allows selection of files/directories with computed visual state icons (`Full`, `Skeleton`, `Mixed`, `Unselected`). Supports direct icon-click toggling, right-click clearing, and double-click to open.
...
* **State Engine:** Tracks context levels strictly at the leaf-node level, computing directory states dynamically via a fast in-memory cache.
