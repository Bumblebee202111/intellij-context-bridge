...
* **Tool Call & Diff Manager:** Relies on automated native UI actions (e.g., "Copy as Markdown") to reliably extract sanitized AI responses. Parses incoming tool-specific XML tags (e.g., `<ide:read_file>`, `<ide:propose_edit>`, `<ide:rename_file>`) to route back to the UI for manual approval, while matching code blocks to local file paths to open IntelliJ's native side-by-side `DiffRequest` window or Refactoring APIs. The system employs a "Code Block Jailbreak" and XML namespacing (`ide:`) to prevent the LLM's Tool Classifier from hijacking the response into a native UI function-call chunk, ensuring tools are extracted as raw text.
