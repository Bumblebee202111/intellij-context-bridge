...
* **Tool Call & Diff Manager:** Relies on automated native UI actions (e.g., "Copy as Markdown") to reliably extract sanitized AI responses. Parses incoming tool-specific XML tags (e.g., `<read_file>`, `<propose_edit>`, `<rename_file>`) to route back to the UI for manual approval, while matching code blocks to local file paths to open IntelliJ's native side-by-side `DiffRequest` window or Refactoring APIs.. The system prompt explicitly "de-weaponizes" these schemas (presenting them as text formatting rules rather than API tools) to prevent the LLM's Tool Classifier from hijacking the response into a native UI function-call chunk.

## 5. Command Engine
* **Slash Commands:** A user-invoked macro system for workflow state transitions (e.g., `/plan`, `/review`). Commands can be stacked. The UI inserts tokens (e.g., `/plan`), and the backend dynamically extracts and interpolates them, wrapping multiple directives in distinct XML semantic boundaries (`<applied_commands>`) before appending the user's specific manual request.
