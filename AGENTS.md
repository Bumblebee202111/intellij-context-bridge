...
- **DOM Structure:** Heavily utilizes custom Angular elements. Core controls often lack standard HTML semantic attributes (e.g., the Run button relies on `<ms-run-button>` rather than `type="submit"`).
...
- **Native Function Calls:** Intercepts native control tokens (heavily RLHF-triggered by "tool" terminology combined with XML schemas) and renders an interactive `<ms-function-call-chunk>`, halting standard text generation.
...
- **Parameter Controls:** Features specific UI components for Model selection (e.g., **Gemini 3.1 Pro Preview** as the primary capability driver, or Gemini 3.8 Flash for speed and independent quota), Thinking Level, and Temperature. Note that changing the selected model automatically resets other parameters to their default states.
