...
* *Context Awareness & Tool Calling:* The AI MUST be provided with XML-based tool schemas (e.g., `<ide:read_file>`). The system MUST intercept these tool calls and present them in the UI for manual user approval or visual review. The prompt MUST forbid the AI from redundantly reading files already provided in their entirety to conserve RPD quota.
