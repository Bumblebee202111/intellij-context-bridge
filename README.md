...
5. **Slash Commands:** Provides modern macro workflows (e.g., `/plan`, `/review`) via Markdown frontmatter files. Supports stacked invocations and reactive mode elevation, keeping the prompt UI clean while dynamically combining complex behaviors via XML semantic boundaries (`<applied_commands>`).
