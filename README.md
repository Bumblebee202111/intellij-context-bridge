...
1. **User-Directed Workflow:** The AI does not autonomously read or write files. The user explicitly selects the context to share, manually approves XML-based AI tool requests (reads, edits, renames, deletions), and safely reviews generated code changes using IntelliJ's native `DiffManager` and Refactoring APIs.
...
4. **Intent-Based Interaction & Prompt Resilience:** Differentiates between read-only analysis ("Ask") and code generation ("Edit") using a unified system prompt combined with XML micro-anchoring (`<user_prompt mode="...">`). Tool schemas are namespaced (`ide:`) and strictly wrapped in Markdown code blocks to bypass aggressive RLHF native function-calling triggers. Code payloads are enforced via `<![CDATA[...]]>` to prevent XML escaping and indentation drift.
5. **Slash Commands & Dynamic Context:** Provides modern macro workflows (e.g., `/commit`, `/review-bugs`, `/plan`) via Markdown frontmatter files with dynamic variable interpolation (e.g., `{{vcs_diff}}`). Includes meta-commands (like `/cmd`) that empower the AI to dynamically design, review, and generate new macros on the fly. Supports stacked invocations and reactive mode elevation, keeping the prompt UI clean while dynamically combining complex behaviors via XML semantic boundaries (`<applied_commands>`).
